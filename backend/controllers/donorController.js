...
import DonorProfile from '../models/DonorProfile.js';
import User from '../models/User.js';
import Center from '../models/Center.js'; // <-- NEW: Import the Center Model
...
// @desc    Get all donation centers (used by CentersPage.jsx)
// @route   GET /api/v1/donors/centers
// @access  Private
const getDonationCenters = asyncHandler(async (req, res) => {
    // Fetches all documents from the Center collection
    const centers = await Center.find({});

    if (centers) {
        // Returns the list of centers to the frontend
        // Frontend expects: { centers: [...] } or just [...]
        res.json(centers); 
    } else {
        res.status(404);
        throw new Error('No donation centers found');
    }
});
...
    createDonorProfile,
    getMyDonorProfile,
    getDonationCenters, // <-- EXPORTED NEW FUNCTION
    getAllDonors,
    updateEligibilityStatus,
};